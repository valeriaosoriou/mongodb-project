...
app.get('/', (req, res) => res.send('Hello World!'))//to see in the browser

app.get('/allProducts', (req,res)=>{ //Requesting info from PRODUCT json
	res.json(product);
});
...
app.post('/addProduct', (req, res)=>{
  //checking if user is found in the db
...
app.delete('/deleteProduct/:id', (req,res)=>{
  const idParam = req.params.id;
  Product.findOne({_id: idParam}, (err, product)=>{ // id refers to mongodb
    if (product){
  ...
  }).catch(err=> res.send(err));// to cath any error
});
